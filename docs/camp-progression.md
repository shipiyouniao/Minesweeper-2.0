...
All 26 purchases cost **39,800**. Settlement alone would require 28 reference Abyss clears or 107 reference Standard clears from zero. However, the 42 one-time goals award **89,220** in total, so these counts do not describe the current collection pace. Three Relaxed clears against three different boss families can already fund the archive when their listed milestone rewards are claimed and saved. See the [closure economy audit](closure-acceptance.md#economy) for the calculation and its assumptions. Goal payouts must be included in any future balance pass; repeated stat levels and cosmetics do not count as distinct gameplay unlocks.
