...
- At each turn start, a walkable echo remains at the player's starting square. The first accepted direct attack records its full actual damage. At turn end the echo follows up only if the player leaves the echo square. Further attacks do not add another echo; it does not activate attack, skill or item triggers again.
...
- Returned spells retain their deadlines, target only the mage and deal 6 damage each. A hit prevents new casting during the following turn; other already announced spells keep their deadlines. The first successful hourglass activation permanently breaks the barrier; direct attacks are blocked until then. Further hourglasses are optional.
...
Direct attacks are blocked until a revealed hourglass is used to return a hostile spell. Breaking this barrier is permanent for that fight, so expending the finite hourglasses cannot leave the boss immune. The former no-hourglass achievement keeps its ID and existing ownership but now requires victory with exactly one hourglass used. Expedition rules revision 10 retires incompatible active journals under the existing camp compensation policy.

A charged echo displays its damage on the board and announces whether the player must move or the follow-up is ready. End-turn follow-ups show an impact number on the boss. Staying on the echo square forfeits that turn's follow-up; fatal hostile spells still resolve first.
