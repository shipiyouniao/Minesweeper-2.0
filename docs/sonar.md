...
Sonar is a standalone information puzzle, available at `?ruleset=sonar`. It shares the Classic mine-placement and reveal engine while owning its own rules, action journal and rankings. Its Expedition adaptation is the released [Echo Warden](echo-warden.md), with separate equipment and arena-loan resources.
