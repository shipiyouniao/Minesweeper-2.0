...
`game/milestones.ts` owns the finite mission/achievement catalog, cumulative accepted-event counters and atomic one-time claims. `milestones.d.ts` defines additive camp progress and exclusive reward contracts. The session updates these counters alongside the journal, never during replay, and snapshots claimed relic unlocks and one owned title into new departures. `titles.d.ts` and `title-effects.ts` own finite title contracts and pure combat, treasure, skill and progression effects. The current rules revision retires incompatible journals with checkpointed extraction. `milestone-template.ts` and `milestone-copy.ts` render progress and reward previews through ordinary camp navigation. See [missions and achievements](milestones.md).
...
Expedition rewards and settlement use a dedicated `ExpeditionDialog`, which owns modal visibility and focus independently of confirmation dialogs. The view automatically opens pending rewards after a floor transition or journal recovery, and displays already committed results after victory, defeat or extraction. The application routes commands through the existing session. See [expedition dialogs and board scrolling](reward-dialog.md) for lifecycle rules and browser regression checks.
...
Expedition envelope version 4 separates the current replay journal from permanent camp progress. `expedition-format.ts` owns the single current rules revision. The persistence boundary retires mismatched journals and returns their checkpointed extraction value to camp; versions 1–3 receive the one-time 200-supply transition compensation. `ExpeditionSession` commits that credit together with journal removal. Only current intents are replayed, and invalid journals cannot discard a valid camp. No historical gameplay engines remain. See [the save policy](save-policy.md).
...
`battle-arena.ts` owns connected shuffled layouts and generation acceptance; `mine-deduction.ts` reads public clue constraints. `battle-turns.ts` owns objective, exposure, damage and action transitions. `combat-build.ts` owns finite equipment/training catalogs, derived stats and the vitality scale. These modules contain no DOM, storage or audio calls. The application snapshots training and the Battle manual license in each departure. The journal rules revision protects replay; rule changes retire older expeditions at the storage boundary.
...
`BoardRightClick` separately owns captured right-mouse events, flags only on release and cancels dragged presses. `Game.safeMarks` stores removable hypotheses; `Expedition.triggeredMines` records actual hazard hits within the known-mine set. Neither notes nor hidden mine bits can authorize walking. `dungeon-chord.ts` batches normal reveal transitions against current reachability and AP, leaving uncertain targets as notes. All four input modes use one finite `BoardInputMode` union. See [board interaction](board-interaction.md).
