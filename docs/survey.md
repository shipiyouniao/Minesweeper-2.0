...
- Double-click a row/column clue to quick-open only that line. On touch screens, double-tap the same clue; keyboard users can focus it and press Enter, Space or C. A single pointer click only selects the clue. Scrolls and taps on different clues never combine into an action.
