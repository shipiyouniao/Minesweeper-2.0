...
test('the seeded six-boss rotation reaches every family and never repeats adjacent checkpoints', () => {
  const kinds = [48, 49, 50, 51, 52, 53].map(
    (seed) =>
      enterEncounter({
  ...
  assert.deepEqual(kinds, ['bastion', 'brood', 'mirror', 'magnetic', 'clock', 'echo'])
