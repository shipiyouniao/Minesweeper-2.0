...
/** Public-clue player proves wins without tools, skills, hourglasses or patched state. */
  ...
  for (let round = 0; round < 500 && run.phase === 'boss'; round++) {
    ...
    for (let depth = 0; depth <= 5; depth++) {
      ...
      for (const plan of beam) {
        ...
        const choices: ExpeditionAction[] = [{ type: 'attack' }]
