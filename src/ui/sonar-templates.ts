...
export function sonarTemplate(language: Language, state: Sonar): string {
  ...
  return `${siteHeaderTemplate(language, 'data-control')}
    ...
    <div class="sonar-layout ${guidanceStyles['sonar-layout']}"><section class="sonar-board-panel ${guidanceStyles['sonar-board-panel']}"><div class="sonar-board-heading ${guidanceStyles['sonar-board-heading']}"><span>${state.game.config.width} × ${state.game.config.height} · ${state.game.config.mines} ${s.mines}</span><button class="icon-button ${sharedStyles['icon-button']}" data-control="zoom" aria-label="${s.zoom}">${icon('globe')}</button></div><div class="sonar-play-area"><div class="board-viewport"><div class="sonar-grid-wrap"><div class="board" role="grid" aria-label="${s.title}"></div><svg class="sonar-overlay" aria-hidden="true"></svg><div class="sonar-pulse" aria-hidden="true"></div></div></div><div class="sonar-pause ${guidanceStyles['sonar-pause']}" hidden><p>${t.paused}</p><button class="primary-button ${sharedStyles['primary-button']}" data-control="pause">${t.resume}</button></div></div></section>
