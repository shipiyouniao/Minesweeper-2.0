...
export function sonarTemplate(language: Language, state: Sonar): string {
  ...
  return `${siteHeaderTemplate(language, 'data-control', 'sonar')}
    ...
    <div class="sonar-layout ${guidanceStyles['sonar-layout']}"><section class="sonar-board-panel ${guidanceStyles['sonar-board-panel']}"><div class="sonar-board-heading ${guidanceStyles['sonar-board-heading']} tw:flex-wrap tw:gap-x-4 tw:gap-y-2"><span class="tw:whitespace-nowrap">${state.game.config.width} × ${state.game.config.height}</span><div class="tw:ml-auto tw:flex tw:items-center tw:gap-3">${mineCounterTemplate(language, state.game)}<button class="icon-button ${sharedStyles['icon-button']}" data-control="zoom" aria-label="${s.zoom}">${icon('globe')}</button></div></div><div class="sonar-play-area"><div class="board-viewport"><div class="sonar-grid-wrap"><div class="board" role="grid" aria-label="${s.title}"></div><svg class="sonar-overlay" aria-hidden="true"></svg><div class="sonar-pulse" aria-hidden="true"></div></div></div><div class="sonar-pause ${guidanceStyles['sonar-pause']}" hidden><p>${t.paused}</p><button class="primary-button ${sharedStyles['primary-button']}" data-control="pause">${t.resume}</button></div></div></section>
    <aside class="sonar-sidebar ${guidanceStyles['sonar-sidebar']}"><section class="sonar-instrument-panel ${guidanceStyles['sonar-instrument-panel']}"><div class="sonar-dial ${guidanceStyles['sonar-dial']}" aria-hidden="true">${sonarIcon()}</div><div class="sonar-counters ${guidanceStyles['sonar-counters']}"></div><p class="sonar-status ${guidanceStyles['sonar-status']}" role="status" aria-live="polite"></p><button class="secondary-button ${sharedStyles['secondary-button']}" data-control="new">${icon('reset')} ${t.restart}</button></section><section class="sonar-log-panel ${guidanceStyles['sonar-log-panel']}"><h2>${s.history}</h2><div class="sonar-log ${guidanceStyles['sonar-log']}"></div><div class="sonar-comparison ${guidanceStyles['sonar-comparison']}"></div></section></aside></div>
