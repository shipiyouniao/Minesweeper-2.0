...
function equipmentTemplate(
  ...
  if (!camp.upgrades.includes('workshop'))
    return `<div class="camp-locked ${campStyles['camp-locked']}">${spriteImage('workshop')}<h2>${upgradeCopy(language, 'workshop').name}</h2><p>${upgradeCopy(language, 'workshop').note}</p><button class="primary-button" data-control="shop-item:workshop">${campPageName(language, 'shop')} ↗</button></div>`

  const spent = equipment.reduce((total, item) => total + equipmentCost(item), 0)
  return `<p class="camp-budget ${campStyles['camp-budget']}">${campLabel(language, 'loadoutBudget')} <strong>${spent} / 3</strong></p><div class="choice-grid">${EQUIPMENT.map((item) => choice(`equipment:${item}`, equipmentCopy(language, item), equipment.includes(item), !equipment.includes(item) && !allowedDeparture(camp, profession, [...equipment, item]), combatSprite(item))).join('')}</div>`
}
...
function shopDetail(
  ...
  return `<aside class="shop-detail ${campStyles['shop-detail']}" id="shop-detail" aria-labelledby="shop-detail-title" style="--shop-rows:${Math.ceil(count / 6)};--compact-rows:${Math.ceil(count / 4)};--detail-row:${Math.floor(index / 3) + 2}">
    ...
    <div class="shop-purchase ${campStyles['shop-purchase']}"><p><strong>${number.format(price)}</strong> ${t.supplies}</p><button class="primary-button" data-control="upgrade:${item}" data-focus-fallback="shop-item:${item}" ${owned || locked || remaining > 0 ? 'disabled' : ''}>${owned ? t.owned : campLabel(language, 'buy')}</button></div>
...
export function campTemplate(
  ...
  return `<section class="camp-panel" data-camp-page="${screen.page}">
    <header class="camp-header ${campStyles['camp-header']}"><div>${screen.page === 'overview' ? '<p class="eyebrow">EXPEDITION / BASE CAMP</p>' : `<button class="text-button camp-back ${campStyles['camp-back']}" data-control="camp-page:overview">← ${campLabel(language, 'back')}</button>`}<h1 tabindex="-1">${campPageName(language, screen.page)}</h1></div><div class="camp-wallet ${campStyles['camp-wallet']}">${spriteImage('treasure')}<div><span>${t.supplies}</span><strong>${number.format(camp.supplies)}</strong></div></div></header>
