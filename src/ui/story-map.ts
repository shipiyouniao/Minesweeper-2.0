...
export function storyMap(state: StoryViewState): string {
  ...
  const names = STORY_ATLAS_SCENES.map((scene) => worldSceneName(lang, scene.id))
  ...
  const unlocked = (index: number): boolean => storyAtlasUnlocked(state.progress, state.run, index)
  let drawing: string
  let landmarks = ''
  if (level === 'local') {
    const content = STORY_ATLAS_SCENES[scene]
    if (!content || !unlocked(scene)) {
        ...
        .map((terrain, index) => {
          ...
          const destination = portal
            ? portal.destination === 'camp'
              ? 3
              : portal.destination === 'north-road'
                ? 4
                : portal.destination === 'northwest-bridge'
                  ? 9
                  : 10
            : scene >= 9
              ? null
    ...
    const terrain =
      level === 'world'
        ? worldTerrain()
        : regionTerrain(!!state.progress.facts?.includes('west-line-restored'))
    const nodes =
      level === 'world'
        ...
        : [
            ...
            .map(
              ([x, y], i) =>
                `<button class="atlas-node ${i === current ? 'is-current' : ''}" data-map-name="${escapeHtml(!unlocked(i) ? message(lang, 'story.map-unvisited') : names[i]!)}" style="--x:${x}%;--y:${y}%" data-story-action="map-scene" data-scene="${i}" ${!unlocked(i) ? 'disabled' : ''}>${i === 3 ? spriteImage('workshop') : i === 1 ? spriteImage('treasure') : `<img src="${import.meta.env.BASE_URL}assets/story/${i === 0 ? 'tree' : 'lantern'}.png" alt="">`}<strong>${!unlocked(i) ? message(lang, 'story.map-unvisited') : names[i]}</strong>${i === current ? `<span>${position}</span>` : ''}</button>`,
            )
            .join('')
  ...
  return `<div class="story-map" data-map-level="${level}" data-map-scene="${scene}"><div class="atlas-toolbar">${back}${scale}</div><div class="atlas-heading"><h3>${title}</h3>${state.progress.facts?.includes('chapter-one-cleared') ? `<p data-chapter-location>${message(lang, 'finale.ending-location')}</p>` : state.progress.facts?.includes('west-line-restored') ? `<p data-west-location>${message(lang, 'finale.location')}</p>` : state.progress.facts?.includes('beacon-recovered') ? `<p data-waterway-location>${message(lang, 'waterway.location')}</p>` : state.progress.facts?.includes('ridge-surveyed') ? `<p data-ridge-location>${message(lang, 'ridge.location')}</p>` : ''}</div><div class="atlas-canvas"><div class="atlas-viewport" data-enter-label="${message(lang, 'story.atlas-enter')}"><div class="atlas-scene">${drawing}</div></div>${legend}</div>${zoom}</div>`
}
