...
export function mirrorHeader(language: Language, encounter: MirrorEncounter): string {
  return `<div class="mirror-heading ${sharedStyles['mirror-heading']}"><h3>${message(language, 'mirror-template.mirror-twins')}</h3><div class="mirror-enemies ${sharedStyles['mirror-enemies']}">${(
    ['dawn', 'dusk'] as const
  )
    .map(
      (side) =>
        `<div class="mirror-enemy ${sharedStyles['mirror-enemy']} ${encounter.active === side ? 'is-active' : ''} ${encounter[side].health === 0 ? 'is-defeated' : ''}" data-realm="${side}">${spriteImage(side === 'dawn' ? 'mirror-dawn' : 'mirror-dusk')}<div><strong class="mirror-enemy-name">${mirrorName(language, side)}</strong><span class="mirror-enemy-health">${encounter[side].health}/${encounter[side].maxHealth}</span><p>${mirrorDefense(language, encounter, side)}</p></div></div>`,
    )
    .join(
      '',
