  ...
  return `${siteHeaderTemplate(language, 'data-control')}
    <main class="survey-main tw:[width:min(94%,1440px)] tw:min-[2400px]:[width:min(90%,1600px)] tw:mx-auto tw:px-0 tw:pb-8 ">
      ...
      <div class="tw:grid tw:gap-5 tw:items-start tw:min-[960px]:grid-cols-[minmax(0,1fr)_280px] tw:min-[1600px]:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside class="survey-sidebar ${panel} tw:p-5 tw:flex tw:flex-col tw:gap-5"><div class="survey-counters tw:grid tw:grid-cols-2 tw:gap-3"></div><div class="tw:border-0 tw:border-t tw:border-solid tw:border-line tw:pt-4"><p class="tw:text-[clamp(14px,0.9vw,18px)] tw:font-medium">${message(language, 'survey.hint')}</p><div class="survey-focus tw:my-3 tw:text-[clamp(14px,0.9vw,18px)] tw:text-accent tw:leading-7" role="status"></div><p class="tw:text-[clamp(12px,0.8vw,16px)] tw:text-muted tw:leading-relaxed">${message(language, 'survey.bookkeeping')}</p>${surveyRunExamplesTemplate()}</div><p class="survey-status tw:text-[clamp(14px,0.9vw,18px)] tw:text-muted" role="status"></p><button class="secondary-button ${sharedStyles['secondary-button']}" data-control="new">${icon('reset')} ${t.restart}</button></aside>
      </div>
