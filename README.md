...
**[Missions and achievements](docs/milestones.md):** select difficulty directly on the camp overview, then pursue 20 missions and 22 achievements for supplies, two exclusive mobility careers, Field radio equipment, eight exclusive relics and [22 titles with build abilities](docs/title-builds.md). Goal currency follows a reduced [milestone budget](docs/milestone-balance.md). Progress persists across runs; each reward can be claimed once.

**Five boss families:** Bastion Guardian, Brood Queen, [Mirror Twins](docs/mirror-twins.md), [Magnetic Knight](docs/magnetic-knight.md) and [Clock Mage](docs/clock-mage.md). Read magnetic push/pull forecasts and lure charges into anchors; against the Clock Mage, escape frozen spell deadlines and use an echo to follow up after retreat. Each boss has its own artwork, arrival scene and mechanics. Reduced-motion mode preserves the tactical forecasts. Read the [mode design](docs/game-modes.md) and [expansion Roadmap](https://github.com/shipiyouniao/Minesweeper-2.0/issues/1) for current rules and planned modes.
