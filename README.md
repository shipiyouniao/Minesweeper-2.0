...
The playable story includes the prologue, shared camp, northern road/quarry/lift route and a complete six-stage main chapter of three authored floors per stage. In [An Answer in the Tower](docs/tower-signal-rescue.md), disconnect powered gates and rescue Nia. In [Ridge Observatory](docs/ridge-observatory.md), route power between instruments and passages to locate the homeward beacon. In [Old Waterway](docs/old-waterway.md), explore larger 15 × 15 to 19 × 17 minefields, drain the chambers and confront the source of the signal. The [Control Room and Northwest Bastion](docs/chapter-one-finale.md) finish the main arc: restore the west line, cross a new persistent bridge, open a camp shortcut and repair the guardian in a tactical encounter. [Knocking in the old mine](docs/quarry-rescue.md) is a separate optional three-floor rescue: clear tracks, route a minecart and bring Toma home.

[Chapter Two opens at Reedbank Camp](docs/chapter-two-opening.md), reached through the western pass after the guardian. Light the pier lantern to enter **Recollection**: choose among five difficulties and select earned exploration mechanics and boss families. Ordinary minefields, relays and power-routing rooms use fresh seeded terrain; boss battles retain their tactical help without arrival dialogue. Both camps share purchases, loadouts, supplies and ordinary achievements, while story stages and Recollection keep separate attempts. The temporary roguelite entrance remains until the lantern is lit, and paused older expeditions can still be resumed. Chapter Two's exploration stages and further side stories remain in development; see [Roadmap II](https://github.com/shipiyouniao/minefarer/issues/55).

The [opening route](docs/opening-route.md) introduces Nia’s absence before the tower, continues the lift investigation on site, and adds quarry controls and a haul shortcut.
...
**[Expedition Sonar](docs/echo-warden.md):** a shop-unlocked, one-point instrument reports regional mine totals and opens only the center. Owned Sonar recharges every 12 safe excavations; Echo Warden lends an independent version that recharges every four.
