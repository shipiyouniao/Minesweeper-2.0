...
The existing **roguelite remains available through a temporary entrance**, including its current difficulty choices, bosses and save. Its final Recollection entrance and story-driven content selection will arrive later. The playable story includes the prologue, shared camp, northern road/quarry/lift route and three authored stages of three floors each. In [An Answer in the Tower](docs/tower-signal-rescue.md), disconnect powered gates and rescue Nia. In [Ridge Observatory](docs/ridge-observatory.md), route power between instruments and passages to locate the homeward beacon. The full chapter is still in progress; see [Roadmap II](https://github.com/shipiyouniao/minefarer/issues/55).
